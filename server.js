        ...
        function(err) {
            ...
            if (salary_data) {
                db.run(`INSERT INTO employee_salaries (employee_id, basic_salary, allowance_leader, allowance_weekend, allowance_overtime, allowance_sosmed, bonus_sales, bonus_other, deduction_late, deduction_absence, deduction_cashadvance, deduction_other) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    ...
                    function(err2) {
                        if (err2) return res.status(400).json({ success: false, message: 'Karyawan berhasil disimpan, tapi gagal menyimpan komponen gaji: ' + err2.message });
                        return res.json({ success: true, message: 'Karyawan dan komponen gaji berhasil ditambahkan!' });
                    }
                );
            } else {
                return res.json({ success: true, message: 'Karyawan berhasil ditambahkan tanpa rincian gaji!' });
            }
        }
    );
});
                ...
                db.get(`SELECT id FROM employee_salaries WHERE employee_id = ?`, [id], (salCheckErr, salRow) => {
                    if (salRow) {
                        db.run(`UPDATE employee_salaries SET basic_salary=?, allowance_leader=?, allowance_weekend=?, allowance_overtime=?, allowance_sosmed=?, bonus_sales=?, bonus_other=?, deduction_late=?, deduction_absence=?, deduction_cashadvance=?, deduction_other=? WHERE employee_id=?`,
                            ...
                            (updateErr) => {
                                if (updateErr) return res.status(400).json({ success: false, message: 'Gagal mengupdate gaji: ' + updateErr.message });
                                return res.json({ success: true, message: 'Data karyawan dan gaji berhasil diperbarui!' });
                            }
                        );
                    } else {
                        db.run(`INSERT INTO employee_salaries (employee_id, basic_salary, allowance_leader, allowance_weekend, allowance_overtime, allowance_sosmed, bonus_sales, bonus_other, deduction_late, deduction_absence, deduction_cashadvance, deduction_other) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                            ...
                            (insertErr) => {
                                if (insertErr) return res.status(400).json({ success: false, message: 'Gagal memasukkan gaji baru: ' + insertErr.message });
                                return res.json({ success: true, message: 'Data karyawan dan gaji berhasil diperbarui!' });
                            }
                        );
                    }
...
app.delete('/api/employees/:id', (req, res) => {
    const { id } = req.params;
    db.run(`DELETE FROM employee_salaries WHERE employee_id = ?`, [id], (errSal) => {
        if (errSal) return res.status(400).json({ success: false, message: 'Gagal menghapus data gaji: ' + errSal.message });
        
        db.run(`DELETE FROM employees WHERE id = ?`, [id], (errEmp) => {
            if (errEmp) return res.status(400).json({ success: false, message: 'Gagal menghapus data karyawan: ' + errEmp.message });
            res.json({ success: true, message: 'Data karyawan beserta gajinya berhasil dihapus.' });
        });
    });
});
                    ...
                    if (type === 'in') {
                        ...
                        if (existing) {
                            db.run(`UPDATE attendance SET clock_in = ?, store_name = ?, shift_name = ?, selfie = ? WHERE id = ?`, 
                                ...
                                (updateErr) => {
                                    ...
                                    res.json({ success: true, message: `✅ Berhasil Clock In pada tanggal ${today} pukul ${nowTime}.` });
                                });
                        } else {
                        ...
                        db.run(`UPDATE attendance SET clock_out = ? WHERE id = ?`, [nowTime, existing.id],
                            (outErr) => {
                                ...
                                res.json({ success: true, message: `✅ Berhasil memperbarui Clock Out terakhir pada pukul ${nowTime}.` });
                            });
                    }
                });
            });
